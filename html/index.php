...
if (isset($_POST['username'])) {
  ...
  $LoginRS__query=sprintf("SELECT p.profileID, p.compName, p.compLogo, p.compAddress, p.smsCredits, p.useSS, p.smsTesting, p.csvUIDcol, p.csvSMScol, p.csvNoCols, p.csvDBtable, p.csvColHeaders, p.compCampaign, p.importScript, l.username, l.password, l.profileID, l.compContact FROM tblclientprofile as p, tblogin as l WHERE p.profileID=l.profileID and l.username=%s AND l.password=%s",
  ...
  if ($loginFoundUser) {
	...
	$_SESSION['originator'] = "Events Team";
	$_SESSION['sesImp'] = 1;
