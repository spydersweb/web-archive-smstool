...
while (($data = fgetcsv($handle,0)) !== FALSE) {
	...
	if ($num<$_SESSION['MM_CSVNoCols']) {
		
		$data = array_pad ( $data, $_SESSION['MM_CSVNoCols'], $_SESSION['MM_CSVNoCols']-$num );
		$num = count($data);
		}
		...
		if ($num==$numcols) { // We expect to import 39 columns, if this is not the case we skip the row
			...
			if ($data[1]) { // Ensure we are importing a row with a unique identifier
				...
				if ($totalRows_rstips == 0) { // If the record has already been imported we skip it
					...
					GetSQLValueString($data[4],"text"), // Mobile Number need to add a number function - formatMobile Function removed for International no.
